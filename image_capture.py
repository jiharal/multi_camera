...
class VideoReader(ImageCapture):
    def __init__(self, input, loop):
        self.loop = loop
...
class CameraReader(ImageCapture):
...
def start_capture(input, loop, camera_resolution=(1280, 720)):
    errors = {InvalidInput: [], OpenError: []}
    for reader in (ImageRead, DirReader, VideoReader):
        try:
